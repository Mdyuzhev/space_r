# T11 — Workshop View (Рисовалка)

**Проект:** `E:\Politech\space_r`
**Волна:** 5 (параллельно с T12, T13)
**Зависимости:** T03, T04
...
`src/views/WorkshopView.vue` — воркшоп Инженера Гайки: Canvas рисовалка с toolbar.
    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="glass-card p-4 mb-4 flex flex-wrap items-center gap-4">
        ...
        <div class="flex gap-2 ml-auto">
          <button @click="undo" :disabled="history.length === 0" class="px-3 py-2 rounded-lg text-sm text-slate-400 hover:text-white border border-slate-700 hover:border-slate-500 disabled:opacity-30 transition-all">↩️ Отмена</button>
          <button @click="clear" class="px-3 py-2 rounded-lg text-sm text-rose-400 hover:text-rose-300 border border-rose-900 hover:border-rose-700 transition-all">🗑️ Очистить</button>
      ...
      <div v-if="savedDrawings.length > 0" class="mt-8">
        <h3 class="font-display font-bold text-white text-xl mb-4">📋 Мои чертежи</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div
            v-for="(drawing, i) in savedDrawings"
            :key="i"
            class="glass-card p-2 group relative"
          >
            <img :src="drawing.data" :alt="`Чертёж ${i+1}`" class="w-full rounded-lg" />
            <p class="text-slate-500 text-xs text-center mt-1">{{ drawing.date }}</p>
            <button
              @click="loadDrawing(drawing.data)"
              class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-slate-950/70 rounded-lg transition-opacity text-white text-sm font-semibold"
            >
              Открыть
            </button>
          </div>
...
const history = ref([])
const savedDrawings = ref(JSON.parse(localStorage.getItem('space_r_drawings') || '[]'))
...
function save() {
  ...
  const date = new Date().toLocaleDateString('ru', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })
  savedDrawings.value.unshift({ data, date })
  if (savedDrawings.value.length > 8) savedDrawings.value = savedDrawings.value.slice(0, 8)
  localStorage.setItem('space_r_drawings', JSON.stringify(savedDrawings.value))
...
function loadDrawing(data) {
  const img = new Image()
  img.onload = () => { saveHistory(); ctx.drawImage(img, 0, 0) }
  img.src = data
}
</script>
```
...
**Результат:** `src/views/WorkshopView.vue` с полной canvas рисовалкой, undo, save, gallery
**Следующий шаг:** T12, T13 (параллельно)
